...
function getOptionalInputValue(input, manifest, id) {
  const parameterConfiguration = manifest.input.find(
    (payload) => payload.id.toLowerCase() === id.toLowerCase()
  );
  ...
  if (
    valueType === interfaces_1.PayloadType.BOOLEAN &&
    typeof value !== "boolean"
  ) {
    throw new Error(`Input value with id ${id} is not of type BOOLEAN.`);
  }
  if (
    valueType === interfaces_1.PayloadType.STRING &&
    typeof value !== "string"
  ) {
    throw new Error(`Input value with id ${id} is not of type STRING.`);
  }
  if (
    valueType === interfaces_1.PayloadType.NUMBER &&
    typeof value !== "number"
  ) {
    throw new Error(`Input value with id ${id} is not of type NUMBER.`);
  }
  if (
    valueType === interfaces_1.PayloadType.MODEL &&
    typeof value !== "string"
  ) {
    throw new Error(`Input value with id ${id} is not of type STRING (MODEL).`);
  }
  if (
    valueType === interfaces_1.PayloadType.CURRENT_MODEL &&
    typeof value !== "string"
  ) {
    throw new Error(
      `Input value with id ${id} is not of type STRING (CURRENT_MODEL).`
    );
  }
  if (valueType === interfaces_1.PayloadType.ENUM) {
    if (typeof value !== "string") {
      throw new Error(
        `Input value with id ${id} is not of type STRING (ENUM).`
      );
    } else if (
      ...
      throw new Error(
        `Input value with id ${id} is not a valid option for ENUM.`
      );
    }
  }
  return value;
